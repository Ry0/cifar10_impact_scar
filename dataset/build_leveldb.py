...
def create_leveldb(name):
  ...
  path = os.path.join('/home/ry0/Workspace/Python/caffe/examples/cifar10_impact_scar', name) #自分の環境に合わせて変更する
  try:
    shutil.rmtree(path)
  except OSError:
    pass
...
def main():
  train_db = create_leveldb('cifar10_impact_scar_train_leveldb')
  test_db = create_leveldb('cifar10_impact_scar_test_leveldb')
  ...
  for seq, (filepath, label) in enumerate(filepath_and_label):
    ...
    db = test_db if seq % 5 == 0 else train_db
